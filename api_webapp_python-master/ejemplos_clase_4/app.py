...
def pulsaciones_historico(name):
    try:
        ...
        print("Obtener gráfico de la persona", name,apellido)       
...
def registro():
    ...
    if request.method == 'POST':
        try:
            ...
            apellidos =str(request.form.get('apellido')).lower()
            pulsos = str(request.form.get('heartrate'))

            if(nombre is None or pulsos or apellidos is None is None or pulsos.isdigit() is False):
                # Datos ingresados incorrectos
                    return Response(status=400)
            time = datetime.now()

            print("Registrar persona", nombre, "del apellido", apellidos, "con pulsaciones", pulsos)
            heart.insert(time, nombre,apellidos, int(pulsos))
