...
class HeartRate(db.Model):
    ...
    def __repr__(self):
        return f"Paciente {self.name} Apellido,{self.apellidos} ritmo cardíaco {self.value}"


def insert(time, name,apellido, heartrate):
    # Crear un nuevo registro de pulsaciones
    pulsaciones = HeartRate(time=time, name=name,apellidos=apellido, value=heartrate)
...
def report(limit=0, offset=0):
    ...
    for result in query:
        ...
        json_result['apellido'] = pulsaciones.apellidos
